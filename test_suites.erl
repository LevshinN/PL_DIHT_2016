...
%% ======================= AGREEMENTS ==============================================
% 1. For 0.6 score 'widthdraw' function defined in module 'atm'
...
-export([main/1]).
-import(atm, [withdraw/2]).

test_suite_0_6() ->
  State0 = [5000, 50, 50, 50, 1000, 5000, 1000, 500, 100],
  {ok, [5000], State1} = atm:widthdraw(5000, State0),
  {ok, [5000, 1000], State2} = atm:widthdraw(6000, State1),
  {request_another_amount, [], State2} = atm:widthdraw(6000, State2),
  {ok, [], State2} = atm:widthdraw(0, State2),
  {ok, [1000, 500, 100, 50, 50, 50], State3} = atm:widthdraw(1750, State2),
  ok = try
         atm:widthdraw(100, State3),
         error_not_raised
       catch
         _: _ -> ok
       end,
...
interact_with_gen_server(Server) ->
  {ok, [5000]} = gen_server:call(Server, {widthdraw, 5000}),
  {ok, [5000, 1000]} = gen_server:call(Server, {widthdraw, 6000}),
  {request_another_amount, []} = gen_server:call(Server, {widthdraw, 6000}),
  {ok, []} = gen_server:call(Server, {widthdraw, 0}),
  {ok, [1000, 500, 100, 50, 50, 50]} = gen_server:call(Server, {widthdraw, 1750}),
  ok = try
         gen_server:call(Server, {widthdraw, 100}),
         error_not_raised
       catch
         exit : _ -> ok
       end,
...
check_history(History) ->
  % There are two correct answers: 'widthdraw 0' processed or not
  case History of
    [5000, 6000, 0, 1750] -> ok;
    [5000, 6000, 1750] -> ok;
    _ -> wrong_history
